...
class GetExchangeRateUseCase {
  ...
  Future<Result<ExchangeRateEntity>> call({
    ...
    if (amount.isEmpty) {
      return const Failure<ExchangeRateEntity>('El monto no puede estar vacío');
    }

    final double? amountValue = double.tryParse(amount);
    if (amountValue == null) {
      return const Failure<ExchangeRateEntity>('El monto debe ser un número válido');
    }

    if (amountValue <= 0) {
      return const Failure<ExchangeRateEntity>('El monto debe ser mayor a 0');
    }

    if (from.id == to.id) {
      return const Failure<ExchangeRateEntity>('Las monedas deben ser diferentes');
    }
